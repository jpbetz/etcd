...
func newConcurrentReadScheduler(lg *zap.Logger, b *backend, maxConcurrentReadTxs uint64) *concurrentReadScheduler {
...
func (r *concurrentReadScheduler) BeginConcurrentReadTxs() {
	...
	for i := uint64(0); i < (r.maxConcurrentReadTxs - concurrentReadTxs); i++ {
		...
		case rch := <-r.readTxCh:
			...
			if err != nil {
				if r.lg != nil {
					r.lg.Fatal("failed to begin read tx", zap.Error(err))
				} else {
					plog.Fatalf("cannot begin read tx (%s)", err)
				}
			}
			rch <- &MonitoredReadTx{r.openReadCounter, &concurrentReadTx{lg: r.lg, tx: rtx}}
		default:
			// no more to create.
			return
		}
